...
\begin{abstract}
Large language model (LLM) agents lose all conversational context when the context window closes.
Existing approaches---raw context dumps, flat note storage, and chunked retrieval-augmented
generation (RAG)---either exhaust the context budget at scale, discard relational structure,
or fail to surface contradictions and superseded decisions.
We present \textbf{Waggle}, a local-first persistent memory system built on a typed knowledge
graph, and \textbf{Recursive Memory Context Assembly (RMCA)}, an algorithm for assembling
compact, high-signal context packs from that graph in response to agent queries.
RMCA decomposes queries into targeted subqueries, retrieves from multiple evidence lanes,
expands the graph along typed edges, resolves update chains and contradictions, and compresses
the result to a configurable token budget.
We evaluate Waggle on the LongMemEval-S benchmark (500 questions, multi-session retrieval)
and on a suite of five synthetic task families adapted from the RLM benchmark taxonomy.
On LongMemEval-S, \texttt{graph\_raw} achieves \textbf{89.0\% Exact@5} and \textbf{97.4\% R@5}
using \texttt{all-MiniLM-L6-v2} embeddings, with a held-out test split confirming no
overfitting (+1.1\,pp gap).
On pairwise conflict reasoning tasks, RMCA achieves perfect scores at all scales (128--2048 nodes)
while raw context dump and single-query retrieval both score 0.0.
LLM-backed answer-level evaluation with Groq \texttt{llama-3.3-70b-versatile} confirms
EM=1.0, F1=1.0, and hallucination rate=0.0 for RMCA on pairwise tasks, consistent across
3 seeds and 2 scales.
Ablation studies confirm that query decomposition is the primary load-bearing RMCA component.
On linear aggregation tasks, RMCA degrades gracefully, exposing a fundamental O($n$) coverage
limit shared by all fixed-budget retrieval systems.
All code, benchmarks, and artifacts are open source.
\end{abstract}
...
scales. This is the clearest demonstration of RMCA's value: the conflict resolution step
(Step 4) surfaces \texttt{contradicts} edges that neither a raw dump nor a single-query
retrieval can identify.
...
\begin{table}[htbp]
\caption{Qualitative Comparison with RLM Paper\\
(NOT numerically comparable --- different datasets, models, scales)}
...
that require:
\begin{itemize}
\item \textbf{Conflict and contradiction detection.} The \texttt{contradicts} and
\texttt{updates} edge resolution in Step 4 is the only mechanism that can surface
``we reversed this decision'' without the agent having to read the entire history.
...
should be replicated across models. The model is an answerer, not an independent human
judge. The \texttt{DeterministicAnswerer} is a reproducible lower-bound evaluator; its
scores are not equivalent to human preference ratings or LLM-judge quality assessments.
...
We presented Waggle, a local-first persistent memory system for AI agents, and RMCA, an
algorithm for assembling compact, high-signal context packs from a typed knowledge graph.
On LongMemEval-S, \texttt{graph\_raw} achieves 89.0\% Exact@5 with no overfitting confirmed
by a held-out split (+1.1\,pp gap). On pairwise conflict reasoning tasks, RMCA achieves
perfect scores at all scales while raw context dump and single-query retrieval both fail
completely, a pattern consistent with the RLM paper's finding that base models score
$<$0.1\% F1 on OOLONG-Pairs while structured recursive assembly recovers to 23--58\% F1.
...
LLM-backed answer-level evaluation with Groq \texttt{llama-3.3-70b-versatile} confirms the
retrieval-level finding: RMCA is the only method achieving EM=1.0, F1=1.0, and hallucination
rate=0.0 on pairwise conflict tasks, consistent across 3 seeds and 2 scales (128, 512 nodes).
Flat baselines either mark the question as \texttt{insufficient\_context} or hallucinate,
while RMCA's structured context pack provides the conflict annotation needed for the LLM to
answer correctly.

Ablation studies confirm that \textbf{decomposition is the primary load-bearing RMCA
component}: disabling it drops pairwise score from 1.0 to 0.0 at both scales across all
seeds. Graph expansion and conflict resolution do not yet show a measurable delta on current
benchmarks due to benchmark construction limitations.
...
contradict each other. RMCA addresses this by treating conflict resolution and temporal
validity as first-class operations in the context assembly pipeline.

Future work includes: (1) replacing heuristic query decomposition with a learned decomposer;
(2) evaluating on the full RLM public benchmark suite (\texttt{trec\_coarse} OOLONG,
OOLONG-Pairs, BrowseComp-Plus, LongBench-v2 CodeQA) with a matching model setup;
(3) measuring end-to-end graph extraction quality on LongMemEval;
(4) hardening the hybrid reranking weights against distribution shift; and
(5) constructing a \texttt{pairwise\_hidden\_edge} benchmark variant with semantically
indistinguishable node labels to causally isolate graph expansion and conflict resolution
benefits.
...
B.~Zhang, J.~Gu, B.~Peng, and others,
``Recursive language models,''
\emph{arXiv preprint arXiv:2512.24601}, 2026.

\bibitem{bertsch2025oolong}
A.~Bertsch, M.~Gormley, G.~Neubig, and M.~R.~Gormley,
``{OOLONG}: Investigating what makes long-context tasks difficult,''
\emph{arXiv preprint arXiv:2501.05173}, 2025.
...
\begin{verbatim}
python benchmarks/ablation_eval.py \
